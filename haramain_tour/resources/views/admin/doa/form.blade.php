...
<head>
    ...
    <title>{{ $doa ? 'Edit' : 'Tambah' }} Doa - Admin Haramain Tour</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .form-group input:focus, .form-group select:focus, .form-group textarea:focus { outline: none; border-color: var(--gold-color); box-shadow: 0 0 0 3px rgba(201,168,76,0.15); }
        .form-group textarea { min-height: 100px; resize: vertical; }
        .form-group textarea.arabic-input { font-family: 'Traditional Arabic', 'Amiri', serif; font-size: 1.3rem; direction: rtl; text-align: right; line-height: 2; min-height: 120px; }
        .form-group .hint { font-size: 0.75rem; color: var(--text-gray); margin-top: 5px; }
...
<body class="{{ ($userSettings['dark_mode'] ?? false) ? 'dark-mode' : '' }}">
    ...
    <main class="main-container">
        <div class="page-header">
            <a href="{{ route('admin.doa.index') }}" class="back-btn"><i class="fa-solid fa-arrow-left"></i></a>
            <div class="page-title">
                <h1><i class="fa-solid fa-hands-praying" style="color: var(--gold-color);"></i> {{ $doa ? 'Edit Doa' : 'Tambah Doa Baru' }}</h1>
                <p>{{ $doa ? 'Perbarui informasi doa' : 'Isi form untuk menambahkan doa baru' }}</p>
            </div>
        </div>

        <div class="form-card">
            <form action="{{ $doa ? route('admin.doa.update', $doa->id) : route('admin.doa.store') }}" method="POST">
                @csrf
                @if($doa) @method('PUT') @endif
                ...
                <div class="form-group">
                    ...
                    <select name="category" required>
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat }}" {{ old('category', $doa->category ?? '') === $cat ? 'selected' : '' }}>
                                {{ ucfirst($cat) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" value="{{ old('title', $doa->title ?? '') }}" placeholder="Contoh: Doa Masuk Masjidil Haram" required>
                    @error('title') <div class="error-msg">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label>Teks Arab</label>
                    <textarea name="arabic" class="arabic-input" placeholder="اكتب الدعاء بالعربية" required>{{ old('arabic', $doa->arabic ?? '') }}</textarea>
                    @error('arabic') <div class="error-msg">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label>Latin / Transliterasi</label>
                    <textarea name="latin" placeholder="Contoh: Bismillahi wash-shalaatu..." required>{{ old('latin', $doa->latin ?? '') }}</textarea>
                    @error('latin') <div class="error-msg">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label>Arti / Terjemahan</label>
                    <textarea name="translation" placeholder="Contoh: Dengan nama Allah..." required>{{ old('translation', $doa->translation ?? '') }}</textarea>
                    @error('translation') <div class="error-msg">{{ $message }}</div> @enderror
                </div>

                <button type="submit" class="btn-submit">
                    <i class="fa-solid fa-save"></i> {{ $doa ? 'Simpan Perubahan' : 'Tambah Doa' }}
                </button>
            </form>
        </div>
    </main>
